...
class AzureAuthorizationCodeBearerBase(SecurityBase):
    def __init__(
        ...
        cloud_base_url: Optional[str] = 'https://login.microsoftonline.com',
        ...
        **Only used for multi-tenant application**
        ...
        self.cloud_base_url: str = cloud_base_url
        ...
        if not self.authorization_url:
            if multi_tenant:
                self.authorization_url = f'{self.cloud_base_url}/common/oauth2/v2.0/authorize'
            else:
                self.authorization_url = (
                    f'{self.cloud_base_url}/{self.openid_config.tenant_id}/oauth2/v2.0/authorize'
                )
        if not self.token_url:
            if multi_tenant:
                self.token_url = f'{self.cloud_base_url}/common/oauth2/v2.0/token'
            else:
                self.token_url = f'{self.cloud_base_url}/{self.openid_config.tenant_id}/oauth2/v2.0/token'
    ...
    def validate(
        ...
        alg = 'RS256'
        log.info(
            'Validating token with arguments:\n'
            'Issuer: %s\n'
            'Key: %s\n'
            'Algorithms: %s\n'
            'Audience: %s\n'
        ...
        log.info('Decoded Token:\n %s', jwt.decode(access_token, key=key, algorithms=[alg], options={"verify_signature": False}))
